  ...
  return (
    <div className="flex flex-col gap-8 py-8 max-w-4xl mx-auto">
      <div className="flex flex-col gap-4">
        <h1 className="text-3xl md:text-4xl font-black text-white border-r-4 border-primary pr-4">الأنشطة القادمة</h1>
        ...
        <div className="flex flex-col gap-6 relative pl-4 md:pl-0 pr-4 md:pr-12">
          ...
          {[1,2,3,4].map(i => (
            <div key={i} className="relative flex items-start gap-6">
               ...
               <div className="w-full bg-card border border-border p-5 rounded-2xl h-24 animate-pulse" />
            </div>
          ))}
        </div>
      ) : (
        <div className="flex flex-col gap-6 relative pl-4 md:pl-0 pr-4 md:pr-12">
          ...
          {plannerEvents?.length === 0 ? (
            <div className="text-center text-muted-foreground py-8 bg-card border border-border rounded-2xl">
              لا توجد أنشطة مجدولة في هذا الشهر
            </div>
          ) : (
